...
// `warringtontalkingtrails://` URL scheme (declared in Info.plist, handled by
// WarringtonTalkingTrailsApp's .onOpenURL) and fake detections can be driven from the
// command line — or just use the ./simulate_beacon.sh wrapper at the repo root:
//
//   xcrun simctl openurl booted "warringtontalkingtrails://fakebeacon?minor=7&distance=2.5"
...
enum FakeBeacon {
    static func handle(url: URL) {
        guard url.scheme == "warringtontalkingtrails", url.host == "fakebeacon" else { return }
        if url.path == "/clear" {
