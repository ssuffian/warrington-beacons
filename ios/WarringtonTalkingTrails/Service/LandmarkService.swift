...
class LandmarkService {
    ...
    public func processData(_ data: WarringtonTalkingTrailsData, imageBase: String = BASE_URL_STRING) {
        queue.sync {
            if let locations = data.locations {
                for location in locations {
                    sitesByMajor[location.beaconMajorCode] = Site(
                        boundaryCoordinates: [],
                        iBeaconUUID: location.iBeaconUUID,
                        altBeaconUUID: location.altBeaconUUID,
                        beaconMajorCode: location.beaconMajorCode
                    )
                }
            } else if let site = data.site {
                sitesByMajor[site.beaconMajorCode] = site
            }
